...
import { ShoppingBag, Package, Plus, Eye } from 'lucide-react'
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.error((err as Error).message || 'Không thể thêm vào giỏ hàng')
    }
  }
      ...
      <article className="group relative flex flex-col overflow-hidden rounded-xl bg-card border border-border transition-all duration-300 hover:shadow-lg hover:shadow-muted/50 hover:-translate-y-0.5">
        ...
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-all duration-300 group-hover:bg-black/20">
            <span className="flex items-center gap-1.5 rounded-full bg-background/90 px-4 py-2 text-xs font-medium opacity-0 shadow-lg backdrop-blur-sm transition-all duration-300 group-hover:opacity-100">
