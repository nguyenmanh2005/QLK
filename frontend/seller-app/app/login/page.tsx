...
function LoginContent() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${SELLER_API}/api/seller/register`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 mt-1">Quản lý cửa hàng của bạn</p>
        </div>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <input type="email" required value={form.email}
                  ...
                  className="w-full h-12 rounded-xl bg-white/10 border border-white/10 px-4 text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all" />
              </div>
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} required
                    ...
                    placeholder="••••••••"
                    className="w-full h-12 rounded-xl bg-white/10 border border-white/10 px-4 pr-12 text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all" />
              ...
              <button type="submit" disabled={loading}
                className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-medium transition-all disabled:opacity-60 flex items-center justify-center gap-2">
